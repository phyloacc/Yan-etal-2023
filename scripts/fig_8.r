...
cat(as.character(Sys.time()), " | Fig8: Adjusting node labels...\n")
...
cat(as.character(Sys.time()), " | Fig8: Assigning target lineages...\n")
...
cat(as.character(Sys.time()), " | Fig8: Reading likelihood files...\n")
...
cat(as.character(Sys.time()), " | Fig8: Reading postz files and generating plots...\n")
...
for(c in 1:3){
  ...
  # Look up the current target and non-target lineages
  ...
  st_opt_postz = readPostZ(st_m1_postz_file, st_m2_postz_file, st_elem_lik_scenario, use_m2=TRUE)
  ...
  gt_opt_postz = readPostZ(gt_m1_postz_file, gt_m2_postz_file, gt_elem_lik_scenario, use_m2=TRUE)
  ...
  gt_non_targ_long = select(st_opt_postz, cur_case_targets[["non.targets"]]$new.labels)
...
cat(as.character(Sys.time()), " | Fig8: Combining plots...\n")
...
if(save_fig){
  figfile = "../figs/fig8.pdf"
  cat(as.character(Sys.time()), " | Fig8: Saving figure:", figfile, "\n")
